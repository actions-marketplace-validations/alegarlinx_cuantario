...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.demo:
        ...
        ap.error("indica una RUTA o usa --demo")
    if not root.is_dir():
    ...
    findings = scan(root, ctx, a.confianza_min)
    ...
    Path(f"{a.salida}_informe.md").write_text(build_report(findings, ctx, target), encoding="utf-8")

    score, _, _ = readiness_score(findings)
    print(f"PQC-Radar {__version__} · {len(findings)} hallazgos en '{target}' · preparación PQC {score}/100")
    for p in PRIORITIES:
        n = sum(f.priority == p for f in findings)
        if n:
            print(f"  {p:<8} {n}")
    print(f"Salidas: {a.salida}_cbom.json · {a.salida}_informe.md")
